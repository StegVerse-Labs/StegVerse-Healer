...
def test_stegbrowser_runtime_consumption_uses_existing_neutral_scheduler_carrier():
    ...
    rows = [row for row in schedule["tasks"] if row.get("reusable_task_id") == "RT-STEGBROWSER-RUNTIME-CONSUMPTION-001"]
    assert len(rows) == 1
    row = rows[0]
    assert row["tracking_task_id"] == "STEG-BROWSER-RUNTIME-CONSUMPTION-001"
...
def test_stegbrowser_binding_reuses_neutral_scheduler_trigger_surface():
