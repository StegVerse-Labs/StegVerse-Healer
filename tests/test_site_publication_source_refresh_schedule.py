...
def test_site_publication_source_refresh_uses_neutral_reusable_scheduler_config():
    ...
    rows = [row for row in schedule["tasks"] if row.get("reusable_task_id") == "RT-SOVEREIGN-SOURCE-REFRESH-001"]
    assert len(rows) == 1
    row = rows[0]
    assert row["tracking_task_id"] == "SITE-PUBLICATION-NATIVE-RUNTIME-EXECUTION-001"
    assert row["cosv_task_vector"] == "50000000102000"
