...
## 2026-09-24 ST-018 resident HB(Δ) scheduling candidate

Source PR: https://github.com/StegVerse-Labs/StegVerse-Healer/pull/102 (DRAFT; no merge authorization). Central existing-identity reconciliation: https://github.com/StegVerse-Labs/.github/issues/2654 (authentic generation-bound AI_SESSION_GATE disposition pending). Current source is candidate only; successful Test Readiness on an exact PR head does not discharge the missing Registry admission or authentic resident execution and Master Records closure requirements.


Task identity: existing `HEALER-RSTD-ST018-LOCAL-TASK-MANAGER-001` / Healer #11; scheduler carrier: `SHWP-HEALER-SOVEREIGN-SCHEDULER-001` / COSV `50000000100000`. The existing worker registry still references `HEALER-TV-TVC-NO-GITHUB-TOKEN-DISPATCH-001` as `HANDOFF_READY`, but the generation-217 canonical Task Registry does not contain that goal or the scoped ST-018 source goal. This is an explicit canonical registration/owner reconciliation condition; source PR is draft until authentic central disposition. No newly invented Goal Task ID is implied.

Candidate implementation on `repair/st018-resident-hb-delta-carrier-001`: remove the fixed UTC hours from ST-018 only; select using independently observed resident heartbeat epoch / generation, validated 10 ms oscillator lineage, six-hour-equivalent 2,160,000-reference threshold, 90,000-reference bounded retry minimum, four attempts per period, and an atomic hashed resident-local last-execution checkpoint. First valid HB observation permits one bootstrap attempt. No time-of-day fallback, no synthetic heartbeat progression or automatic production activation. Fail closed for absent/invalid/regressing HB or malformed checkpoint. The enclosing WorkerCoordinator retains admission, claim and fence; an HB observation cannot grant execution or Master Records custody. Each authentic parent execution must retain the exact eligibility/target/checkpoint outcome; governed transition closure remains separately required. Source and tests must receive exact-head CI before any merge; production completion requires authentic resident scheduler + Master Records evidence where applicable.
